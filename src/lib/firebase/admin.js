...
import { getClientEnv, getServerEnv, parseServiceAccountKey } from '../env.js';
...
export async function verifyIdToken(idToken) {
  ...
  if (typeof window !== 'undefined') {
    throw new Error('verifyIdToken must not run on the client');
  }
  ...
  // Validate env eagerly to provide a clear error in production
  const serverEnv = getServerEnv();
  const clientEnv = getClientEnv();

  // Ensure the service account key can be parsed (even though we use Google certs)
  parseServiceAccountKey(serverEnv.FIREBASE_SERVICE_ACCOUNT_KEY);
  ...
  const projectId = clientEnv.NEXT_PUBLIC_FIREBASE_PROJECT_ID || process.env.FIREBASE_PROJECT_ID;
  if (!projectId) {
